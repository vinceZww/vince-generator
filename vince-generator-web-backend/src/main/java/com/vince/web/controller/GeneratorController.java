...
import com.vince.maker.meta.MetaValidator;
...
public class GeneratorController {
    ...
    public BaseResponse<Page<GeneratorVO>> listGeneratorVOByPage(@RequestBody GeneratorQueryRequest generatorQueryRequest,
        ...
        Page<Generator> generatorPage = generatorService.page(new Page<>(current, size),
                generatorService.getQueryWrapper(generatorQueryRequest));
        stopWatch.stop();
        System.out.println("查询生成器耗时(ms)：" + stopWatch.getTotalTimeMillis());

        stopWatch = new StopWatch();
        stopWatch.start();
        Page<GeneratorVO> generatorVOPage = generatorService.getGeneratorVOPage(generatorPage, request);
        stopWatch.stop();
        System.out.println("查询关联数据耗时(ms)：" + stopWatch.getTotalTimeMillis());
...
//        });
    ...
    public void downloadGeneratorById(long id, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        if (StrUtil.isBlank(filepath)) {
            throw new BusinessException(ErrorCode.PARAMS_ERROR, "产物包不存在");
        }
        //追踪事件
    ...
    public void useGenerator(@RequestBody GeneratorUseRequest generatorUseRequest, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        stopWatch.stop();
        System.out.println("下载耗时(ms)：" + stopWatch.getTotalTimeMillis());
        ...
        stopWatch.stop();
        System.out.println("解压耗时(ms)：" + stopWatch.getTotalTimeMillis());
        ...
        stopWatch.stop();
        System.out.println("写数据文件耗时(ms)：" + stopWatch.getTotalTimeMillis());
        // 执行脚本
        // 找到脚本文件所在路径
        // 要注意：如果不是 win 系统，要找generator文件，而不是bat
        ...
        try {
            ...
            stopWatch.stop();
            System.out.println("执行脚本耗时(ms)：" + stopWatch.getTotalTimeMillis());
        } catch (Exception e) {
            e.printStackTrace();
        ...
        stopWatch.stop();
        System.out.println("压缩结果耗时(ms)：" + stopWatch.getTotalTimeMillis());
    ...
    public void makeGenerator(@RequestBody GeneratorMakeRequest generatorMakeRequest, HttpServletRequest request, HttpServletResponse response) throws IOException {
        ...
        try {
            ...
            stopWatch.stop();
            System.out.println("下载文件耗时(ms)：" + stopWatch.getTotalTimeMillis());
        } catch (Exception e) {
        ...
        try {
            ...
            stopWatch.stop();
            System.out.println("制作耗时(ms)：" + stopWatch.getTotalTimeMillis());
        } catch (Exception e) {
    ...
    public String getCacheFilePath(long id, String distPath) {
        String propertyPath = System.getProperty("user.dir");
        String tempDirPath = String.format("%s/.temp/cache/%s", propertyPath, id);
    ...
    private static String getPageCacheKey(GeneratorQueryRequest generatorQueryRequest) {
        ...
        String key = "generator:pagr:" + base64;
        return key;
